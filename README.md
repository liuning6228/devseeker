...
DevSeeker 是一款**自主 AI 编码 Agent**，不满足于"自动补全"——它能理解整个代码库、自主规划实施步骤、调用 34+ 工具读写文件执行命令，并像资深工程师一样在行动前先思考。

支持 DeepSeek / OpenAI / Anthropic / Qwen / Ollama 等多种模型，主推 **DeepSeek V4（编码）+ Qwen-VL-Max（视觉）** 双模型智能路由。

<div align="center">
  <img src="logo.png" alt="DevSeeker" width="120">
</div>
...
- **三粒度回滚**：单步 → 当前轮次 → 整个会话
- **时间线面板**：`DevSeeker: Show Checkpoint Timeline` 命令打开可视化时间线，支持 Compare Diff
...
- `glob`：按文件路径匹配注入
- **双源加载**：全局 `~/.devseeker/rules/` + 工作区 `.devseeker/rules/`
...
# 国内用户可通过 GitCode 加速：
# git clone https://gitcode.com/a6228/devseeker.git
cd devseeker
npm install
npm run build
...
打包完成后会在项目根目录生成 `devseeker-0.1.0-linux-x64.vsix` 和 `devseeker-0.1.0-win32-x64.vsix`。

**安装 VSIX：**
```bash
# Linux
code --install-extension devseeker-0.1.0-linux-x64.vsix

# Windows
code --install-extension devseeker-0.1.0-win32-x64.vsix
```
...
`File → Preferences → Settings` → 搜 `devSeeker`，填写：

| 设置 | 必填 | 说明 |
|------|------|------|
| `devSeeker.models.llm.level1.apiKey` | **是** | 主力 LLM 的 API Key（默认 DeepSeek） |
| `devSeeker.models.vllm.level1.apiKey` | 否 | 视觉模型 Key（截图需要，默认 Qwen-VL） |
| `devSeeker.webResearch.tavily.apiKeys` | 否 | 联网搜索（Tavily，英文） |
| `devSeeker.webResearch.bocha.apiKeys` | 否 | 联网搜索（博查，中文） |

只填第一项 LLM Key 即可开始编码。

#### 2. 打开面板

**方式一 · 底部状态栏**：点击 VSCode 底部状态栏右侧的 `$(rocket) DevSeeker` 按钮。

**方式二 · 命令面板**：`Ctrl+Shift+P` → **DevSeeker: Open Panel** → 在主编辑器列打开聊天面板。
...
| 命令 | 作用 |
|------|------|
| `DevSeeker: Open Panel` | 打开聊天面板 |
| `DevSeeker: Show Logs` | 打开 runtime.log |
| `DevSeeker: Reindex Codebase` | 手动重建语义索引 |
| `DevSeeker: Revert to Checkpoint…` | 从快照列表回滚 |
| `DevSeeker: Show Checkpoint Timeline` | 查看快照时间线 |
| `DevSeeker: Export Session (md/json)` | 导出会话 |
| `DevSeeker: Show Cost Panel` | 查看今日用量 |
| `DevSeeker: Show Rules Panel` | 查看当前生效的 Rules |
| `DevSeeker: Show Hooks Panel` | 查看 Hooks 状态 |
...
| `Ctrl+Shift+L` | 选中代码后询问 DevSeeker |
...
<p align="center">
  ...
  <a href="https://github.com/liuning6228/devseeker/issues">Issues</a>
</p>
...
DevSeeker is an **autonomous AI coding agent** for VSCode. It understands your entire codebase, plans implementation steps, calls 34+ tools to read/write/execute/search, and thinks before acting -- like a senior engineer.
...
1. Set `devSeeker.models.llm.level1.apiKey` in VSCode settings
2. Click `$(rocket) DevSeeker` in the bottom status bar, or `Ctrl+Shift+P` → **DevSeeker: Open Panel**
3. Start coding — ask anything, DevSeeker will plan and execute
...
```bash
git clone https://github.com/liuning6228/devseeker.git
# For users in China, use GitCode mirror:
# git clone https://gitcode.com/a6228/devseeker.git
cd devseeker
npm install
npm run build
...
After packaging, you'll get `devseeker-0.1.0-linux-x64.vsix` and `devseeker-0.1.0-win32-x64.vsix` in the project root.
