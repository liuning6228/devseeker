...
/** 知识库根目录（默认 `<ws>/.dualmind/knowledge`）。 */
export function defaultKnowledgeRoot(workspaceRoot: string): string {
  return joinPath(workspaceRoot, '.dualmind', 'knowledge');
}

/** 知识库 BM25 索引快照文件（默认 `<ws>/.dualmind/knowledge-index.json`）。 */
export function defaultKnowledgeIndexPath(workspaceRoot: string): string {
  return joinPath(workspaceRoot, '.dualmind', 'knowledge-index.json');
}
