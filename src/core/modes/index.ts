  ...
  switch (mode) {
    case 'agent':
      lines.push(
        ...
        'If you encounter a bug, error, or the user reports a failure,',
        'you MUST immediately call `switch_mode(target_mode_id="debug")` to use the evidence-first troubleshooting loop.',
        '',
    ...
    case 'plan':
      lines.push(
        ...
        '',
        '## Spec Workflow (第三代工作流 — 大型/复杂任务优先)',
        '当任务涉及多模块、高风险或用户明确要求 Spec 流程时，使用五阶段 Spec 工作流：',
        '',
        '**Stage 1 — Requirement（需求梳理）**:',
        '  使用 `AskUserQuestion` 向用户收集需求细节（目标、边界、约束、验收条件）。',
        '  产出写入 `.devseeker/specs/<feature>/spec.md` 的 `## 需求` 部分。',
        '',
        '**Stage 2 — Explore（代码探索）**:',
        '  `Agent({preset:\'explorer\', mode:\'fork\', description:\'Explore for spec\', prompt:\'...\'})`',
        '  探索代码库，了解受影响模块和接口。结果写入 spec 的 `## 方案` 部分。',
        '',
        '**Stage 3 — Design（方案设计）**:',
        '  综合探索结果，设计实现方案。更新 spec 的 `## 方案` 部分。',
        '',
        '**Stage 4 — Task Split（任务拆分）**:',
        '  将方案拆解为离散的、可在单个 Agent turn 内完成的任务列表。',
        '  更新 spec 的 `## 任务` 部分，格式：`- [ ] 1. 任务描述`。',
        '',
        '**Stage 5 — Verify（校验）**:',
        '  `Agent({preset:\'verifier\', mode:\'fork\', description:\'Verify spec\', prompt:\'...\'})`',
        '  验证 spec 中引用的文件/符号是否存在。',
        '',
        'Spec 状态流转：draft → approved → completed（不可逆）。',
        'Spec 文件通过 `<spec>` XML 块注入 system prompt，Agent 实施时会参考。',
      );
      break;
    case 'debug':
      lines.push(
        ...
        '',
        'Step 6 — MEMORY ACCUMULATION: After successful verification, record the BUG pattern.',
        '  - Use `update_memory` to save this pattern as `common_pitfalls_experience`.',
        '  - Include: symptom, root cause, fix location, verification method.',
        '  - This helps future debugging sessions by providing quick pattern matching.',
        '',
